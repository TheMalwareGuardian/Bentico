...
void FunctionsTechniques_ShowTechniquesMenu()
{
	...
	printf("    * list <technique>\n");
	for (int i = 0; i < STRUCTURES_TECHNIQUES_CMD_CAT_TOTAL; ++i)
	{
		printf("        %-10s - %s\n", Globals_Techniques_CategoryKeys[i], Globals_Techniques_CategoryNames[i]);
	}
	printf("    * list all\n");
}
...
{
	// ---------------------------------------------------------------------------------------------------------------------
	// Loop through all technique categories and print their commands in table format
	for (int i = 0; i < STRUCTURES_TECHNIQUES_CMD_CAT_TOTAL; ++i)
	{
		UtilsTable_PrintTableByCategory(i);
	}
}
...
{
	// ---------------------------------------------------------------------------------------------------------------------
	// Search for a matching technique keyword (case-insensitive)
	for (int i = 0; i < STRUCTURES_TECHNIQUES_CMD_CAT_TOTAL; ++i)
	{

		// -----------------------------------------------------------------------------------------------------------------
		// Match found, print the command table for this category
		if (_stricmp(name, Globals_Techniques_CategoryKeys[i]) == 0)
		{
			UtilsTable_PrintTableByCategory((STRUCTURES_TECHNIQUES_COMMAND_CATEGORY)i);
			return;
		}
	}
	...
	printf(" \033[0;31m[!] Unknown technique.\033[0m\n");
}
