...
int main() {
	...
	// ---------------------------------------------------------------------------------------------------------------------
	// Variables
	HANDLE port = NULL;					// Handle to the communication port
	DWORD bytesReceived = 0;			// Stores the number of bytes received from the driver
	PCHAR buffer = "user message";		// Message to be sent to the driver
	char recBuffer[50] = { 0 };			// Buffer to store the response from the driver
