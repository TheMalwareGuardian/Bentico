...
// Define operation callbacks (empty as no specific operations are filtered)
...
const FLT_REGISTRATION FilterRegistration = {
	...
	FLT_REGISTRATION_VERSION,		// Version of the filter driver registration
	0,								// Flags (0 means default behavior)
	NULL,							// Contexts (NULL because no specific context is used)
	OperationCallbacks,				// Operation callbacks
	MinifilterUnload,				// Unload routine for cleanup
	NULL, NULL, NULL, NULL,			// Unused callbacks for IRP operations
	NULL, NULL, NULL, NULL			// Unused callbacks for name generation, instance setup, etc.
};
...
/**
	@brief		Handles connection requests from user-mode applications to the communication port.
				...
				PFLT_PORT					https://microsoft.github.io/windows-docs-rs/doc/windows/Wdk/Storage/FileSystem/Minifilters/struct.PFLT_PORT.html
	@param		ClientPort					Handle to the client communication port.


				PVOID						https://learn.microsoft.com/en-us/windows/win32/winprog/windows-data-types#PVOID
	@param		ServerPortCookie			Context information associated with the server port.


				PVOID						https://learn.microsoft.com/en-us/windows/win32/winprog/windows-data-types#PVOID
	@param		Context						Optional context from the user-mode application.


				ULONG						https://learn.microsoft.com/en-us/windows/win32/winprog/windows-data-types#ULONG
	@param		Size						Size of the context data.


				PVOID						https://learn.microsoft.com/en-us/windows/win32/winprog/windows-data-types#PVOID
	@param		ConnectionCookie			Connection-specific context.
...
/**
	@brief		Handles disconnection requests from the user-mode application.
				...
				PVOID						https://learn.microsoft.com/en-us/windows/win32/winprog/windows-data-types#PVOID
	@param		ConnectionCookie			Connection-specific context (unused).
**/
VOID
...
/**
	@brief		Handles send and receive operations for the communication port.
				...
				This function processes messages received from the user-mode application and responds with a predefined kernel message.

				PVOID						https://learn.microsoft.com/en-us/windows/win32/winprog/windows-data-types#PVOID
	@param		PortCookie					Port-specific context.
