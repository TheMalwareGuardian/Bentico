...
BOOLEAN Global_CallbackRegistered = FALSE;
BOOLEAN Global_CallbackExRegistered = FALSE;
...
)
{
	...
	if (Global_CallbackExRegistered) {
...
)
{
	...
	// Success
	if (NT_SUCCESS(status)) {
		Global_CallbackExRegistered = TRUE;
	}
	// Failed
	else
	{
		...
		// Cleanup previously registered callback to avoid dangling references
		if (Global_CallbackRegistered) {
